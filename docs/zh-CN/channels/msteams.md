...
- 通过 Teams 私信、群聊或频道与 OpenKrab 交流。
...
- 启动时，OpenKrab 将团队/频道和用户允许列表名称解析为 ID（当 Graph 权限允许时）
  并记录映射；未解析的条目保持原样。
...
在配置 OpenKrab 之前，你需要创建一个 Azure Bot 资源。
...
OpenKrab 通过快速返回并主动发送回复来处理这个问题，但非常慢的响应仍可能导致问题。
...
默认情况下，OpenKrab 仅从 Microsoft/Teams 主机名下载媒体。使用 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 允许任何主机）。
...
上传的文件存储在配置的 SharePoint 站点默认文档库中的 `/OpenKrabShared/` 文件夹中。

## 投票（Adaptive Cards）

OpenKrab 将 Teams 投票作为 Adaptive Cards 发送（没有原生 Teams 投票 API）。
