...
```bash
openclaw nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from OpenKrab"}'
```
...
OpenKrab can run a **headless node host** (no UI) that connects to the Gateway
