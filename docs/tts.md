...
OpenKrab can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere OpenKrab can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, OpenKrab defaults
...
- If `provider` is **unset**, OpenKrab prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
Discord note: `/tts` is a built-in Discord command, so OpenKrab registers
