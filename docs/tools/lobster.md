---
title: Lobster
summary: "Typed workflow runtime for OpenKrab with resumable approval gates."
description: Typed workflow runtime for OpenKrab — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets OpenKrab run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: OpenKrab runs one Lobster tool call and gets a structured result.
...
OpenKrab launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In OpenKrab tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the OpenKrab Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), OpenKrab keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls OpenKrab tools that do.
