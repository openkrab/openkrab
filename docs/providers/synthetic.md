---
summary: "Use Synthetic's Anthropic-compatible API in OpenKrab"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OpenKrab registers it as the
...
Note: OpenKrab's Anthropic client appends `/v1` to the base URL, so use
