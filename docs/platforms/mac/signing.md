...
- inject build metadata into Info.plist: `OpenKrabBuildTimestamp` (UTC) and `OpenKrabGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `OpenKrabBuildTimestamp`: ISO8601 UTC at package time
- `OpenKrabGitCommit`: short git hash (or `unknown` if unavailable)
...
TCC permissions are tied to the bundle identifier _and_ code signature. Unsigned debug builds with changing UUIDs were causing macOS to forget grants after each rebuild. Signing the binaries (ad‑hoc by default) and keeping a fixed bundle id/path (`dist/OpenKrab.app`) preserves the grants between builds, matching the VibeTunnel approach.
