...
Accessed via `OpenKrabPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenKrabConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenKrabConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenKrabConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenKrabConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenKrabConfig): string;
  };
};
```
...
- Add `api.runtime` to `OpenKrabPluginApi` with the surface above.
