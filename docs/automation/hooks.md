...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in OpenKrab. See [Webhook Hooks](/automation/webhook) or use `openclaw webhooks` for Gmail helper commands.
...
3. **Bundled hooks**: `<openclaw>/dist/hooks/bundled/` (shipped with OpenKrab)
