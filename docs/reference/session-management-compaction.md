...
This document explains how OpenKrab manages sessions end-to-end:
...
OpenKrab is designed around a single **Gateway process** that owns session state.
...
OpenKrab persists sessions in two layers:
...
OpenKrab resolves these via `src/config/sessions.ts`.
...
OpenKrab intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OpenKrab consumes the events, but Pi decides when to compact).
...
OpenKrab also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OpenKrab bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OpenKrab leaves it alone.
...
OpenKrab supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OpenKrab strips/suppresses this in the delivery layer.

As of `2026.1.10`, OpenKrab also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OpenKrab uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OpenKrab’s
