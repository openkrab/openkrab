...
OpenKrab integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenKrab owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenKrab spawns `imsg rpc` as a child process (legacy iMessage integration).
