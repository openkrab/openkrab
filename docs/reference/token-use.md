---
summary: "How OpenKrab builds prompt context and reports token usage + costs"
read_when:
...
OpenKrab tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OpenKrab assembles its own system prompt on every run. It includes:
...
For images, OpenKrab downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OpenKrab session logs.
...
`cacheWrite`. If pricing is missing, OpenKrab shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OpenKrab can
...
Anthropic's 1M context window is currently beta-gated. OpenKrab can inject the
