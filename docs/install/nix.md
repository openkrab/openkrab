...
The recommended way to run OpenKrab with Nix is via **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** — a batteries-included Home Manager module.
...
OpenKrab reads JSON5 config from `OPENCLAW_CONFIG_PATH` and stores mutable data in `OPENCLAW_STATE_DIR`.
...
```
apps/macos/Sources/OpenKrab/Resources/Info.plist
```
