...
2. Link a bot account:
   - `signal-cli link -n "openkrab"` then scan the QR in Signal.
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `openkrab pairing approve signal <PAIRING_CODE>`.
...
- Approve via:
  ...
  - `openkrab pairing approve signal <CODE>`
...
- Config validation errors after edits: run `openkrab doctor --fix`.
