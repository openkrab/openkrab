...
```bash
openkrab plugins install @openkrab/mattermost
```

Local checkout (when running from a git repo):

```bash
openkrab plugins install ./extensions/mattermost
```
...
- Approve via:
  - `openkrab pairing list mattermost`
  - `openkrab pairing approve mattermost <CODE>`
...
Use these target formats with `openkrab message send` or cron/webhooks:
