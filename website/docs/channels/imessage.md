...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.openkrab/scripts/imsg-ssh",
...
- Full Disk Access is required for the process context running openkrab/`imsg` (Messages DB access).
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`openkrab pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs openkrab/`imsg`.
