...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `openkrab doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `openkrab logs --follow`
...
```
    ...
    - `~/.openkrab/telegram/sticker-cache.json`
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `openkrab channels status` warns when config expects unmentioned group messages.
    - `openkrab channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `openkrab logs --follow` for skip reasons
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `openkrab doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `openkrab doctor --fix` can resolve legacy `@username` entries to IDs.
