...
Cron jobs are persisted on the Gateway host at `~/.openkrab/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openkrab cron add/edit` or the cron
...
- Job store: `~/.openkrab/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openkrab/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openkrab system event`](/cli/system).
