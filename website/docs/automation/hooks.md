...
- Extend openkrab's behavior without modifying core code
...
During onboarding (`openkrab onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.openkrab/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<openkrab>/dist/hooks/bundled/` (shipped with openkrab)
...
Hook packs can ship dependencies; they will be installed under `~/.openkrab/hooks/<id>`.
Each `openkrab.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `openkrab hooks install` installs dependencies with `npm install --ignore-scripts`
...
- **Managed hooks** (`~/.openkrab/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.openkrab/hooks/my-hook
cd ~/.openkrab/hooks/my-hook
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.openkrab/workspace`)
...
```yaml
metadata: { "openkrab": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "openkrab": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.openkrab/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.openkrab/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.openkrab/hooks/my-hook/handler.ts
   ```
