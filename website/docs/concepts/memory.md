...
`~/.openkrab/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
Remote embeddings **require** an API key for the embedding provider. openkrab
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.openkrab/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.openkrab/agents/<agentId>/qmd/` (config + cache + sqlite DB).
