...
This is handy when you need to toggle obscure settings without editing `openkrab.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[openkrab]"
...
- **Global `--dev` (profile):** isolates state under `~/.openkrab-dev` and
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.openkrab/logs/raw-stream.jsonl
```
