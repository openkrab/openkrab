...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up openkrab?](#whats-the-recommended-way-to-install-and-set-up-openkrab)
  ...
  - [I can't access docs.openkrab.ai (SSL error). What now?](#i-cant-access-docsopenkrabai-ssl-error-what-now)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart openkrab?](#i-closed-my-terminal-on-windows-how-do-i-restart-openkrab)
  ...
  - [ELI5: `openkrab gateway restart` vs `openkrab gateway`](#eli5-openkrab-gateway-restart-vs-openkrab-gateway)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/openkrab/openkrab-*.log | head -1)"
   ```
...
[https://github.com/openkrab/openkrab/issues](https://github.com/openkrab/openkrab/issues)
[https://github.com/openkrab/openkrab/pulls](https://github.com/openkrab/openkrab/pulls)
...
- `openkrab status`: quick snapshot of gateway/agent health + basic config.
- `openkrab models status`: checks provider auth + model availability.
- `openkrab doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `openkrab status --all`, `openkrab logs --follow`,
`openkrab gateway status`, `openkrab health --verbose`.
...
- **Tailscale Serve** (recommended): keep bind loopback, run `openkrab gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run `openkrab gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
under `~/.openkrab/` (for example `~/.openkrab/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-openkrab-store-its-data),
...
[https://github.com/openkrab/openkrab/blob/main/CHANGELOG.md](https://github.com/openkrab/openkrab/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docs.openkrab.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.openkrab.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.openkrab.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsopenkrabai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/openkrab/openkrab/tree/main/docs](https://github.com/openkrab/openkrab/tree/main/docs)
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://openkrab.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://openkrab.ai/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://openkrab.ai/install.ps1](https://openkrab.ai/install.ps1)
...
```bash
curl -fsSL https://openkrab.ai/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://openkrab.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```
...
`openkrab onboard` is the recommended setup path. In **local mode** it walks you through:
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `openkrab models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `openkrab models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `openkrab models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `openkrab.json`.

Steps:

1. Enable the plugin: `openkrab plugins enable google-gemini-cli-auth`
2. Login: `openkrab models auth login --provider google-gemini-cli --set-default`
...
- Use `openkrab nodes status` / `openkrab nodes list` to see it.
...
- DM your bot, then run `openkrab logs --follow` and read `from.id`.
...
Showcase: [https://openkrab.ai/showcase](https://openkrab.ai/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.openkrab/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.openkrab/openkrab.json`). Precedence is `<workspace>/skills` > `~/.openkrab/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.
...
Not directly. macOS skills are gated by `metadata.openkrab.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
...
3. Override the skill metadata (workspace or `~/.openkrab/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "openkrab": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.openkrab` + your workspace directory).
...
Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.openkrab/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.openkrab`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.openkrab`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.openkrab/skills`).

Default workspace is `~/.openkrab/workspace`, configurable via:
...
Do **not** commit anything under `~/.openkrab` (credentials, sessions, tokens).
...
key. **Recommended:** run `openkrab configure --section web` to store it in
...
- Daemons read env vars from `~/.openkrab/.env` (or the service environment).
...
- Gateway is running: `openkrab gateway status`
- Gateway health: `openkrab status`
- Channel health: `openkrab channels status`
...
`openkrab agent --message ... --deliver`, targeting a chat where the other bot
...
- Restore from backup (git or a copied `~/.openkrab/openkrab.json`).
- If you have no backup, re-run `openkrab doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `openkrab config set` for small changes.
- Use `openkrab configure` for interactive edits.
...
1. Put the missing keys in `~/.openkrab/.env` so they're picked up even when the service doesn't inherit your shell env.
...
1. Put the token in `~/.openkrab/.env`:
...
- **Disk growth:** sessions + transcripts live under `~/.openkrab/agents/<agentId>/sessions/`.
...
- Use `openkrab doctor` to spot stray workspaces and profile mismatches.
...
- `openkrab models set ...` (updates just model config)
- `openkrab configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.openkrab/openkrab.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `openkrab doctor` to repair.
...
- Run `openkrab agents add <id>` and configure auth during the wizard.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.openkrab/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.openkrab/agent/*` (migrated by `openkrab doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.openkrab/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `openkrab models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `openkrab models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `openkrab models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.openkrab/.env` on the **gateway host**.
...
Use `openkrab gateway status` and trust these lines:
...
- `openkrab gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `openkrab.control.settings.v1`.
...
Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.openkrab.*`, `openkrab-gateway-<profile>.service`, `openkrab Gateway (<profile>)`).
...
1. Is the Gateway running? `openkrab gateway status`
2. Is the Gateway healthy? `openkrab status`
3. Does the UI have the right token? `openkrab dashboard`
...
If you installed the service, use the gateway commands. Use `openkrab gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [openkrab assistant setup](/start/openkrab) and [Agent send](/tools/agent-send).
...
- Default behavior on DM-capable channels is **pairing**:
  ...
  - Pending requests are capped at **3 per channel**; check `openkrab pairing list <channel>` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `openkrab doctor` to surface risky DM policies.
