...
```mermaid
flowchart TD
  A[openkrab is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `openkrab gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Browser tool fails">
    ...
    - `openkrab` profile starts or `chrome` relay has an attached tab.
