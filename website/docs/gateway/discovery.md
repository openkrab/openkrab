...
The design goal is to keep all network discovery/advertising in the **Node Gateway** (`openkrab gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_openkrab-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `openkrab` entrypoint or binary)
