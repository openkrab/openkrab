...
- **WhatsApp**: `~/.openkrab/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.openkrab/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.openkrab/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.openkrab/credentials/oauth.json`
...
`openkrab security audit` warns when this setting is enabled.
...
- `~/.openkrab/openkrab.json`: `600` (user read/write only)
- `~/.openkrab`: `700` (user only)

`openkrab doctor` can warn and offer to tighten these permissions.
...
The Gateway broadcasts its presence via mDNS (`_openkrab-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `openkrab doctor --generate-gateway-token`.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openkrab", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.openkrab/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `openkrab` profile).
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openkrab gateway` process.
...
1. Check Gateway logs: `/tmp/openkrab/openkrab-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openkrab/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `openkrab security audit --deep` and confirm critical findings are resolved.
