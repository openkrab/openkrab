...
`_openkrab-gw._tcp` in your discovery domain without multicast.
...
Only the Gateway advertises `_openkrab-gw._tcp`.
...
- `transport=gateway`
- `cliPath=<path>` (optional; absolute path to a runnable `openkrab` entrypoint)
...
The iOS node uses `NWBrowser` to discover `_openkrab-gw._tcp`.
