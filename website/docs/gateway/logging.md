...
- Default rolling log file is under `/tmp/openkrab/` (one file per day): `openkrab-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.openkrab/openkrab.json`:
  - `logging.file`
  - `logging.level`
...
`openkrab gateway` supports a per-gateway style switch:
