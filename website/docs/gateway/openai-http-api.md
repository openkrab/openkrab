...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `openkrab agent`), so routing/permissions/config match your Gateway.
...
- `x-openkrab-session-key: <sessionKey>` to fully control session routing.
