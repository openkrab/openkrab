...
- During heartbeat runs, openkrab treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openkrab sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, openkrab skips the heartbeat run entirely (no model call).
