  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.openkrab/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `openkrab doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["openkrab-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-openkrab",
          requireMention: false,
...
Mattermost ships as a plugin: `openkrab plugins install @openkrab/mattermost`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@openkrab", "openkrab"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@openkrab"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `openkrab.json`).
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/openkrab" } },
}
```
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.openkrab/sandboxes",
        docker: {
          ...
          apparmorProfile: "openkrab-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "openkrab-sandbox-browser:bookworm-slim",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
- `none`: per-scope sandbox workspace under `~/.openkrab/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.openkrab/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.openkrab/workspace-home" },
      { id: "work", workspace: "~/.openkrab/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.openkrab/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.openkrab/workspace-family",
...
```json5
{
  session: {
    ...
    store: "~/.openkrab/agents/{agentId}/sessions/sessions.json",
    maintenance: {
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.openkrab/settings/tts.json",
      elevenlabs: {
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:openkrab`   | All built-in tools (excludes provider plugins)                                           |
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `openkrab onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `openkrab onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `openkrab onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `openkrab onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `openkrab onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `openkrab onboard --auth-choice minimax-api`.
...
- Loaded from `~/.openkrab/extensions`, `<workspace>/.openkrab/extensions`, plus `plugins.load.paths`.
...
```json5
{
  browser: {
    ...
    profiles: {
      openkrab: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.openkrab/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-openkrab-token: <token>`.
...
Writes a unicast DNS-SD zone under `~/.openkrab/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `openkrab dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.openkrab/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.openkrab/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/openkrab/openkrab.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/openkrab/openkrab-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `openkrab doctor --fix` can strip unknown keys).
