...
- `openkrab gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `openkrab doctor` reports no blocking config/service issues.
- `openkrab channels status --probe` shows connected/ready channels.
