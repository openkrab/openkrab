...
```json5
{
  agent: { workspace: "~/.openkrab/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.openkrab/openkrab.json` and you can DM the bot from that number.
...
```json5
{
  ...
  routing: {
    groupChat: {
      mentionPatterns: ["@openkrab", "openkrab"],
      historyLimit: 50,
    },
    queue: {
      mode: "collect",
  ...
  session: {
    ...
    store: "~/.openkrab/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/openkrab" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
