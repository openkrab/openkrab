...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/openkrab.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/openkrab.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/openkrab.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/openkrab.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/openkrab.json"
```
...
```bash
# List current IPs
fly ips list -a my-openkrab

# Release public IPs
fly ips release <public-ipv4> -a my-openkrab
fly ips release <public-ipv6> -a my-openkrab
...
fly ips allocate-v6 --private -a my-openkrab
```
...
fly proxy 3000:3000 -a my-openkrab
