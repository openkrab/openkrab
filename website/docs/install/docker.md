...
```bash
docker build -t openkrab:local -f Dockerfile .
docker compose run --rm openkrab-cli onboard
...
```bash
docker compose run --rm openkrab-cli dashboard --no-open
docker compose run --rm openkrab-cli devices list
docker compose run --rm openkrab-cli devices approve <requestId>
```
...
`/home/node/.openkrab`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/openkrab-config /path/to/openkrab-workspace
```
...
```bash
docker compose run --rm openkrab-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm openkrab-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm openkrab-cli channels add --channel discord --token "<token>"
```
...
- The gateway container is the source of truth for sessions (`~/.openkrab/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.openkrab/sandboxes`
...
This builds `openkrab-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `openkrab-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "openkrab-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `openkrab-sandbox-browser:bookworm-slim` using
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-openkrab-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-openkrab-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-openkrab-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/openkrab/openkrab/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
